...
@views.route("/manage-payroll",methods=["GET","POST"])
def manage_payroll():
    ...
    if request.method=="POST":
        ...
        # Call the required organization function to handle the record
        if response["submit"]=="Create": ORGANIZATION.create_payroll(deepcopy(response))
        if response["submit"]=="Update": ORGANIZATION.update_payroll(deepcopy(response)) 
        if response["submit"]=="Delete": ORGANIZATION.remove_payroll(deepcopy(response))
